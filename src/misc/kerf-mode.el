...
;;(defvar kerf-program-name "/Users/kevin/Desktop/kerf-alias/kerf" "kerf executable name.")
(defvar kerf-program-name "/home/scott/src/kerf-source/kerf" "kerf executable name.")
...
(defvar kerf-mode-map
  (let ((m (make-sparse-keymap)))
    ...
    ;(define-key m (kbd "C-c C-l") 'kerf-load-file)
    ;; (define-key m (kbd ")") 'kerf-electric-rparen)
    ;; (define-key m (kbd "]") 'kerf-electric-rbrace)
    ;; (define-key m (kbd "}") 'kerf-electric-rcurly)
    ;; (define-key m (kbd "\"") 'kerf-electric-quote)
    m)
  "Keymap for kerf mode.")

(defconst kerf-verbnames
  (concat "\\<\\("
      (regexp-opt
       '(
	 "abs" "acos" "add" "and" "ascend" "asin" "asof_join" "atan" "atlas" "atom" "avg"
	 "between" "btree" "bucketed" 
	 "car" "cdr" "ceil" "char" "close_socket" "combinations" "cos" "cosh" "count" "count_nonnull" "count_null" "create_table_from_csv" "create_table_from_psv" "create_table_from_tsv" "cross" 
	 "deal" "def" "delete_keys" "descend" "dir_ls" "display" "distinct" "divide" "dlload" "dotp" "drop"
	 "enlist" "enum" "enumerate" "equal" "equals" "erf" "erfc" "eval" "except" "exit" "exp" "explode"
	 "filter" "first" "flatten" "float" "floor" "function"
	 "global" "globals" "greater" "greatereq" 
	 "has_column" "hash" "hashed" "has_key"
	 "ident" "ifnull" "implode" "in" "index" "indexed" "int" "intersect" "isnull"
	 "join" "json_from_kerf" 
	 "kerf_from_json" "kerf_type" "kerf_type_name" "KEY" 
	 "last" "left_join" "len" "less" "lesseq" "lg" "lines" "ln" "load" "log" "lsq"
	 "map" "match" "mavg" "max" "maxes" "mcount" "median" "meta_table" "min" "mins" "minus" "minv" "mkdir" "mmax" "mmin" "mmul" "mod" "msum"
	 "negate" "negative" "ngram" "NONNULL" "not" "noteq" "noteq" "now" "now_date" "now_time"
	 "open_socket" "open_table" "or" "order" "out"
	 "part" "permutations" "plus" "pow" "PRIMARY"
	 "rand" "range" "read_from_path" "read_striped_from_path" "read_table_from_csv" "read_table_from_delimited_file" "read_table_from_fixed_file" "read_table_from_tsv" "rep" "repeat" "reserved" "reverse" "rsum" "run"
	 "search" "seed_prng" "send_async" "send_sync" "setminus" "shift" "shuffle" "sin" "sinh" "sleep" "sort" "sort_debug" "sqrt" "split" "stamp_diff" "std" "string" "subtract" "sum" "system"
	 "tables" "take" "tan" "tanh" "times" "timing" "tolower" "toupper" "transpose" "trim" "type_null"
	 "uneval" "union" "unique" "UNIQUE"
	 "var"
	 "which" "write_csv_from_table" "write_delimited_file_from_table" "write_striped_to_path" "write_text" "write_to_path"
	 "xbar" "xkeys" "xvals"
         ) t)
      "\\)\\>" )
)
...
;;    ("`\"[^\"]*\"" . 'kerf-symbol-face)
    ...
    ("\"[^\"]*\"" . 'kerf-string-face)
    ("\'[^\']*\'" . 'kerf-string-face)
;;    (";" . 'font-lock-keyword-face)  ;; no such thing in Kerf
;;    ("_[a-zA-Z]+" . 'kerf-builtin-face) ;; no such thing in Kerf
    ("[a-zA-Z][a-zA-Z0-9_]*:" . 'kerf-variable-binding-face)
;;    ("[a-zA-Z][a-zA-Z0-9]*" . 'kerf-variable-face)
    ("-?[0-9]+\\(\\.?[0-9]*\\)?\\([eE][+-]?[0-9]+\\)?" . 'kerf-number-face)
;;    ("\\(`\\)\\([a-zA-Z][a-zA-Z0-9]*\\)" 
;;     (1 'kerf-builtin-face)
;;     (2 'kerf-symbol-face))
    ("[!#$%&*+,.;<=/>?@^|~-:]:?" . 'kerf-builtin-face)
;;     (kerf-verbnames . 'kerf-builtin-face) ;; not understanding how this gets evaluated; if I stick a constant here, it works, if I stick a global, it doesn't...
